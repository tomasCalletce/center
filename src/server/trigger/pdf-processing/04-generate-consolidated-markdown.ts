...
export const generateConsolidatedMarkdownTask = task({
  ...
  maxDuration: 300,
  run: async (payload: GenerateMarkdownPayload): Promise<GenerateMarkdownResult> => {
    ...
    try {
      ...
      const sortedImages = payload.processedImages.sort((a, b) => a.pageNumber - b.pageNumber);
      
      // Generate consolidated content
      let consolidatedContent = `# ${payload.originalFileName.replace('.pdf', '')} - Document Analysis\n\n`;
      
      // Add document metadata section
      const allElements = [...new Set(sortedImages.flatMap(img => img.metadata.elements))];
      const averageConfidence = sortedImages.reduce((sum, img) => sum + img.metadata.confidence, 0) / sortedImages.length;
      ...
      consolidatedContent += `- **Average Confidence**: ${(averageConfidence * 100).toFixed(1)}%\n`;
      ...
      for (const image of sortedImages) {
        ...
        consolidatedContent += `**Confidence**: ${(image.metadata.confidence * 100).toFixed(1)}%  \n`;
        consolidatedContent += `**Elements Found**: ${image.metadata.elements.join(', ') || 'None'}  \n`;
      ...
      consolidatedContent += `- **Successfully Processed**: ${sortedImages.filter(img => img.metadata.confidence > 0).length} pages\n`;
      consolidatedContent += `- **Failed Processing**: ${sortedImages.filter(img => img.metadata.confidence === 0).length} pages\n`;
      ...
      const markdownFileName = `${payload.originalFileName.replace('.pdf', '')}_consolidated.md`;
